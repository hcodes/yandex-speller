var yaspeller = require('../lib/yandex-speller'),
...
describe('checkText()', function() {
    ...
    it('ignoreUppercase on', function(done) {
        ...
        }, {lang: 'ru', options: {ignoreUppercase: true}});
    });

    it('ignoreUppercase on, html', function(done) {
        var text = getFile('./test/texts/ignore_uppercase_html.txt');
        yaspeller.checkText(text, function(err, data) {
            assert.equal(err, null);
            assert.equal(data.length, 0);
            done();
        }, {lang: 'ru,en', format: 'html', options: {ignoreUppercase: true}});
    });
    ...
    it('findRepeatWords on', function(done) {
        ...
        }, {lang: 'ru', options: {findRepeatWords: true}});
    });
...
describe('checkTexts()', function() {
    it('ignoreUppercase on, html', function(done) {
        ...
        }, {lang: 'ru,en', options: {ignoreUppercase: true}});
    });
});
